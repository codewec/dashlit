...
В релизную сборку встраиваются Git-тег и commit. Установленная версия отображается в футере, а при нативной установке её также можно проверить командой `dashlit --version`. DashLit обращается к GitHub только в сессии администратора и кеширует результат на 12 часов; обычные пользователи не получают информацию о доступном релизе. Ошибка проверки не влияет на запуск и работу DashLit. Установите `UPDATE_CHECK_ENABLED=false`, чтобы запретить этот внешний запрос.
