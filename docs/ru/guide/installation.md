...
Скрипт создаёт непривилегированный контейнер Debian 13 с 1 ядром CPU, 512 МБ памяти, диском 4 ГБ, сетью DHCP через `vmbr0` и автоматическим запуском. Затем внутри контейнера вызывается обычный Linux-установщик. DashLit слушает порт `80` и доступен по адресу `http://IP_КОНТЕЙНЕРА` без указания порта.
