...
Откройте `http://localhost:3000` и зарегистрируйте первую учётную запись — она получит права администратора. Не публикуйте созданный файл `.env` и не добавляйте его в Git.
...
создайте каталог до запуска контейнера и разрешите запись всем. Это самый простой вариант, при котором каталог остаётся доступен пользователю хоста:
...
Docker не меняет владельца каталогов хоста, подключённых через bind mount. Если `./data` отсутствует, Docker обычно создаёт его как `root:root`, из-за чего непривилегированный процесс DashLit не может туда писать. В системах с SELinux также используйте `./data:/data:Z`.
