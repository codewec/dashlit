...
<AlertDialog.Root bind:open>
  <AlertDialog.Portal>
    ...
    <AlertDialog.Content
      class="dialog-content fixed left-1/2 top-1/2 z-50 w-[min(28rem,calc(100vw-1.5rem))] -translate-x-1/2 -translate-y-1/2 rounded-card border border-border bg-surface p-5 shadow-2xl outline-none"
    >
      ...
      <div class="mt-5 flex justify-end gap-2">
        <AlertDialog.Cancel
          class="rounded-btn border border-border bg-transparent px-3.5 py-2 text-sm text-text hover:bg-surface-2"
          disabled={loading}>Cancel</AlertDialog.Cancel
        >
        <AlertDialog.Action
          type="button"
