...
{#if src}
  <img
    ...
    height={size}
    {alt}
    class="shrink-0 object-contain {isDark && isIconify ? 'brightness-0 invert' : ''} {className}"
    style="width:{size}px;height:{size}px"
    loading="lazy"
  />
{:else}
  <span
    class="inline-flex shrink-0 items-center justify-center rounded-md bg-surface-2 text-text-subtle {className}"
