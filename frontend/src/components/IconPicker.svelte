<script lang="ts">
  ...
  $effect(() => {
    ...
    debounce = setTimeout(() => {
      void api
        .searchIconifyIcons(q)
        .then(
          (found) => {
            if (sequence === searchSequence) iconifyResults = found
          },
          () => {
        ...
        .finally(() => {
          if (sequence === searchSequence) searchingIconify = false
        })
      void api
        .searchSelfhstIcons(q)
        .then(
          (found) => {
            if (sequence === searchSequence) selfhstResults = found
          },
          () => {
        ...
        .finally(() => {
          if (sequence === searchSequence) searchingSelfhst = false
        })
    }, 280)
  })
  ...
  <div class="relative min-h-10">
    {#if sourceTab === 'search'}
      <div class="flex items-center gap-2">
        <div
          class="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-border-soft {iconPreviewClass(variant)}"
        >
          {#if value}
        ...
        <div class="absolute left-0 right-0 top-full z-60 mt-1 max-h-48 overflow-y-auto rounded-xl border border-border bg-surface p-2 shadow-xl">
          {#if results.length > 0}
            ...
            <p class="mt-2 text-center text-[10px] text-text-subtle">
              Icons by <a class="underline hover:text-text-muted" href="https://selfh.st/icons/" target="_blank" rel="noreferrer">selfh.st/icons</a> (CC
              BY 4.0) and Iconify
            </p>
            {#if searching}
    ...
    {:else if sourceTab === 'url'}
      <div class="flex h-10 items-center gap-2">
        ...
        <button
          type="button"
          class="h-10 shrink-0 rounded-btn bg-primary px-4 text-sm font-medium text-white hover:bg-primary-hover"
