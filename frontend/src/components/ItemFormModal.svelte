  ...
  <form class="space-y-3" onsubmit={submit}>
    ...
    <div class="rounded-btn border border-border bg-bg-elevated p-3">
      <div class="flex items-center justify-between gap-3">
        ...
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="shrink-0 text-text-subtle transition-transform {form.pingEnabled && availabilityExpanded ? 'rotate-90' : ''}" aria-hidden="true"><path d="m9 18 6-6-6-6" /></svg>
          <span><span class="block text-sm text-text">Availability check</span><span class="block text-xs text-text-subtle">Check the service every 30 seconds</span></span>
        </button>
        <Switch.Root checked={form.pingEnabled} onCheckedChange={(value) => setPingEnabled(!!value)} class="peer inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full bg-border transition data-[state=checked]:bg-primary">
      ...
      {#if form.pingEnabled && availabilityExpanded}
        <div class="mt-3 space-y-3 border-t border-border-soft pt-3">
          <label class="block">
            ...
            />
            <span class="mt-1 block text-xs text-text-subtle">Use a different internal or health-check address without changing the link destination.</span>
          </label>
          <label class="flex items-center justify-between gap-3">
            <span><span class="block text-sm text-text">Skip TLS verification</span><span class="block text-xs text-text-subtle">Allow self-signed or mismatched certificates</span></span>
            <Switch.Root checked={form.pingSkipTls} onCheckedChange={(value) => (form.pingSkipTls = !!value)} class="peer inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full bg-border transition data-[state=checked]:bg-primary">
              <Switch.Thumb class="pointer-events-none block h-5 w-5 translate-x-0.5 rounded-full bg-white shadow transition-transform data-[state=checked]:translate-x-5" />
            </Switch.Root>
          </label>
          <label class="flex items-center justify-between gap-3">
            ...
            <Switch.Root checked={form.pingOnlyDown} onCheckedChange={(value) => (form.pingOnlyDown = !!value)} class="peer inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full bg-border transition data-[state=checked]:bg-primary">
              <Switch.Thumb class="pointer-events-none block h-5 w-5 translate-x-0.5 rounded-full bg-white shadow transition-transform data-[state=checked]:translate-x-5" />
            </Switch.Root>
          </label>
        </div>
