...
export const api = {
  ...
  register: (username: string, password: string, importLegacy = false) =>
    request<{ token: string; user: User }>('/auth/register', {
      method: 'POST',
      body: JSON.stringify({ username, password, importLegacy }),
    }),
  ...
  setDefault: (id: string, isDefault: boolean) => request(`/dashboards/${id}/set-default`, { method: 'POST', body: JSON.stringify({ isDefault }) }),
  cloneDashboard: (id: string) => request<Dashboard>(`/dashboards/${id}/clone`, { method: 'POST' }),
  ...
  importDashboard: (data: unknown) => request<Dashboard>('/dashboards/import', { method: 'POST', body: JSON.stringify(data) }),
  cloneGroup: (id: string) => request<Group>(`/groups/${id}/clone`, { method: 'POST' }),
  cloneItem: (id: string) => request<Item>(`/items/${id}/clone`, { method: 'POST' }),
  ...
  searchIconifyIcons: (query: string) => request<IconSearchResult[]>(`/icons/search/iconify?q=${encodeURIComponent(query)}`),
  searchSelfhstIcons: (query: string) => request<IconSearchResult[]>(`/icons/search/selfhst?q=${encodeURIComponent(query)}`),
