...
export function iconSearchTitle(result: IconSearchResult): string {
  const variant = result.variant === 'color' ? 'Color' : result.variant === 'monochrome' ? 'Monochrome' : ''
...
export function applyIconSelection(result: IconSearchResult, variant: IconVariant, pairedValue: string): { value: string; pairedValue: string } {
...
export function combineIconResults(selfhst: IconSearchResult[], iconify: IconSearchResult[]): IconSearchResult[] {
