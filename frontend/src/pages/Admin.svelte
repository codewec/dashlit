...
<AppLayout dashboards={dashList} currentSlug="" showSearch={false}>
  <div class="space-y-6">
    ...
    {:else if overview}
          ...
          <table class="w-full min-w-2xl text-left text-sm">
            <thead class="text-xs text-text-muted"
              ><tr class="border-b border-border-soft"
                ><th class="px-5 py-3 font-medium">Login</th><th class="px-5 py-3 font-medium">Role</th><th class="px-5 py-3 font-medium"
                  >Sign-in methods</th
                ><th class="px-5 py-3 font-medium">Dashboards</th><th class="px-5 py-3 text-right font-medium">Actions</th></tr
              ></thead
            >
            <tbody>
                ...
                <tr class="border-b border-border-soft last:border-0 hover:bg-surface-2">
                  ...
                  <td class="px-5 py-3"
                    ><div class="flex justify-end gap-2">
                      <button
                        type="button"
                        class="rounded-btn border border-border px-2.5 py-1.5 text-xs hover:bg-surface"
                        onclick={() => openUser(account)}>Edit</button
                      ><button
                        type="button"
                        disabled={account.id === $user?.id}
                        class="rounded-btn px-2.5 py-1.5 text-xs text-danger hover:bg-danger-soft disabled:cursor-not-allowed disabled:opacity-40"
                        onclick={() => ask(`Delete user “${account.username}” and all their dashboards?`, () => deleteUser(account))}>Delete</button
                      >
                    </div></td
                  >
                </tr>
      ...
      <section class="rounded-card border border-border-soft bg-surface">
        <div class="border-b border-border-soft px-5 py-4">
          ...
          <p class="mt-1 text-xs text-text-muted">
            The system default is shown to unauthenticated visitors when public, or to signed-in users without their own dashboards when visibility is
            “users”. A private dashboard cannot be the system default.
          </p>
        </div>
        <div class="overflow-x-auto">
          <table class="w-full min-w-232 text-left text-sm">
            <thead class="text-xs text-text-muted"
              ><tr class="border-b border-border-soft"
                ><th class="px-5 py-3 font-medium">Dashboard</th><th class="px-5 py-3 font-medium">Owner</th><th class="px-5 py-3 font-medium"
                  >Description</th
                ><th class="px-5 py-3 font-medium">Privacy</th><th class="px-5 py-3 font-medium">System default</th><th
                  class="px-5 py-3 text-right font-medium">Actions</th
                ></tr
              ></thead
            >
            <tbody>
              {#each overview.dashboards as dashboard (dashboard.id)}
                <tr class="border-b border-border-soft last:border-0 hover:bg-surface-2">
                  <td class="px-5 py-3"
                    ><a href="#/{dashboard.slug}" class="flex items-center gap-2 font-medium text-text hover:text-primary"
                      >{#if dashboard.icon}<Icon icon={dashboard.icon} iconDark={dashboard.iconDark} size={20} />{/if}<span>{dashboard.name}</span></a
                    ></td
                  >
                  ...
                  <td class="px-5 py-3"
                    >{#if dashboard.isMain}<span class="text-primary">Current</span>{:else}<button
                        type="button"
                        disabled={dashboard.privacy === 'private'}
                        title={dashboard.privacy === 'private' ? 'Private dashboards cannot be system defaults' : 'Set as system default'}
                        class="rounded-btn border border-border px-2.5 py-1.5 text-xs hover:bg-surface disabled:cursor-not-allowed disabled:opacity-40"
                        onclick={() => setSystemDefault(dashboard)}>Set default</button
                      >{/if}</td
                  >
                  <td class="px-5 py-3 text-right"
                    ><button
                      type="button"
                      class="rounded-btn px-2.5 py-1.5 text-xs text-danger hover:bg-danger-soft"
                      onclick={() => ask(`Delete dashboard “${dashboard.name}”?`, () => deleteDashboard(dashboard))}>Delete</button
                    ></td
                  >
                </tr>
              {/each}
      ...
      <section class="rounded-card border border-border-soft bg-surface">
        <div class="border-b border-border-soft px-5 py-4">
          <h2 class="text-sm font-semibold text-text">Environment flags</h2>
          <p class="mt-1 text-xs text-text-muted">
            Read-only effective values loaded from the process environment or .env file. Secrets are never exposed.
          </p>
        </div>
        <div class="overflow-x-auto">
          <table class="w-full min-w-2xl text-left text-sm">
            <thead class="text-xs text-text-muted"
              ><tr class="border-b border-border-soft"
                ><th class="px-5 py-3 font-medium">Key</th><th class="px-5 py-3 font-medium">Value</th><th class="px-5 py-3 font-medium"
                  >Description</th
                ></tr
              ></thead
            >
            <tbody>
              {#each Object.entries(overview.flags) as [name, enabled]}
                <tr class="border-b border-border-soft last:border-0 hover:bg-surface-2">
                  ...
                  <td class="px-5 py-3"
                    ><span
                      class="inline-block rounded-full px-2 py-0.5 text-xs {enabled ? 'bg-primary/15 text-primary' : 'bg-surface-2 text-text-subtle'}"
                      >{enabled ? 'true' : 'false'}</span
                    ></td
                  >
...
<Modal bind:open={editOpen} title="Edit user">
  <form class="space-y-3" onsubmit={saveUser}>
    <label class="block"
      ><span class="mb-1 block text-xs text-text-muted">Login</span><input
        class="w-full rounded-btn border border-border bg-bg-elevated px-3 py-2 text-sm"
    ...
    >
    <label class="block"
      ><span class="mb-1 block text-xs text-text-muted">New password</span><input
        type="password"
        class="w-full rounded-btn border border-border bg-bg-elevated px-3 py-2 text-sm"
    ...
    {#if editingUser?.hasOIDC}
      <div class="rounded-lg bg-danger-soft p-3 text-sm text-danger">
        This account is linked to OIDC. Saving changes will remove that link. The user will need a password or a new OIDC registration to sign in
        again.
      </div>
      <label class="flex items-start gap-2 text-sm text-text"
        ><input type="checkbox" class="mt-1" bind:checked={resetOIDC} /><span>I understand that the OIDC link will be reset.</span></label
      >
    {/if}
    <div class="flex justify-end gap-2 pt-2">
      <button type="button" class="rounded-btn px-3 py-2 text-sm text-text-muted" onclick={() => (editOpen = false)}>Cancel</button><button
        type="submit"
        disabled={savingUser}
        class="rounded-btn bg-primary px-3 py-2 text-sm font-medium text-primary-fg disabled:opacity-60">{savingUser ? 'Saving…' : 'Save'}</button
      >
    </div>
  </form>
</Modal>
