...
pct exec "$ctid" -- bash -c "install -d -m 0755 /etc/systemd/system/container-getty@1.service.d && rm -f /etc/systemd/system/container-getty@1.service.d/autologin.conf && printf '%s\n' '[Service]' 'ExecStart=' 'ExecStart=-/sbin/agetty --autologin root --noclear --keep-baud tty%I 115200,38400,9600 \$TERM' > /etc/systemd/system/container-getty@1.service.d/override.conf && systemctl daemon-reload && systemctl restart container-getty@1.service"
